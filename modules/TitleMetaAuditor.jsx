/**
 * Local SEO & AEO Pro — Title & Meta Auditor
      ...
      setResult({
        ...
        pages: [
          ...
          },
          {
            ...
            title: { current: "Fire Alarm Monitoring Services", length: 30, issues: ["No city name", "No brand"], suggested: `Fire Alarm Monitoring in ${city || "St. Charles"}, MO — 24/7 Central Station | ${businessName || "Citywide Alarms"}` },
          ...
          },
          {
            ...
            title: { current: "Contact Us", length: 10, issues: ["Way too short", "No city, no keyword, no brand"], suggested: `Contact ${businessName || "Citywide Alarms"} — ${city || "St. Charles"} Alarm & Security Company` },
            meta: { current: "(missing)", length: 0, issues: ["Meta description completely missing"], suggested: `Reach ${businessName || "Citywide Alarms"} in ${city || "St. Charles"}, MO. Call, email, or schedule a free on-site security assessment. Available Mon–Sat 8am–6pm.` }
          },
        ],
        summary: `Most pages are missing city names in both the title and meta description — a critical local SEO issue. Three pages have meta descriptions under 50 characters, and one page has no meta description at all. Fixing these alone could move rankings within 2-4 weeks.`
      });
    }
    setRunning(false);
  };
    ...
    <div style={{ maxWidth: 640, fontFamily: "var(--font-sans)" }}>
      <div style={{ background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, padding: "14px 16px", marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
        ...
        <button onClick={run} disabled={running} style={{ padding: "8px 14px", background: running ? "transparent" : MODULE_COLOR, border: `0.5px solid ${MODULE_COLOR}`, borderRadius: 6, color: running ? MODULE_COLOR : "#0B0E16", fontSize: 12, fontWeight: 500, cursor: running ? "not-allowed" : "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
          {running ? "Auditing..." : result ? "Re-run →" : "Audit Titles & Meta →"}
        </button>
      </div>
                      ...
                      return (
                        <div key={type} style={{ padding: "8px 10px", background: "var(--color-background-primary)", borderRadius: 7 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                            ...
                            <span style={{ fontSize: 9, color: lengthColor(data.length, type) }}>{data.length} chars {type === "title" ? "(ideal 50–60)" : "(ideal 150–160)"}</span>
                          </div>
                          <div style={{ fontSize: 11, color: "var(--color-text-secondary)", marginBottom: 4 }}>Current: {data.current || "(missing)"}</div>
                          {(data.issues || []).map((issue, j) => <div key={j} style={{ fontSize: 10, color: "#FBBF24", marginBottom: 2 }}>⚠ {issue}</div>)}
                          {data.suggested && (
      ...
      {!result && !running && (
        <div style={{ textAlign: "center", padding: "36px 20px", background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, color: "var(--color-text-secondary)", fontSize: 12 }}>
          Audits every title tag and meta description — generates optimized replacements for each page
        </div>
      )}
    </div>
  );

/**
 * LocalRank Pro — Competitor Intelligence
...
Generate 3 competitors. Be specific to this industry and city.`,
      ...
      setResult({
        competitors: [
          { name: names[0], label: names[0], threat: "high", overallScore: 78, gbpScore: 82, reviewCount: 94, averageRating: 4.7, estimatedMonthlyTraffic: 1240, topKeywords: [`alarm company ${city||"st charles"}`, `fire alarm installation`, `security monitoring`], strengths: ["Strong GBP with 94 reviews", "Dominant in Google 3-Pack for core terms"], weaknesses: ["No city landing pages beyond main area", "Website loads slowly (4.2s)"], gapToExploit: "They have no content targeting surrounding cities — O'Fallon, St. Peters, Wentzville are all unclaimed.", websiteScore: 61 },
          { name: names[1], label: names[1], threat: "medium", overallScore: 55, gbpScore: 58, reviewCount: 31, averageRating: 4.2, estimatedMonthlyTraffic: 620, topKeywords: [`alarm monitoring`, `home security`, `fire alarm`], strengths: ["Good pricing page", "Active on social media"], weaknesses: ["Only 31 GBP reviews — very beatable", "No FAQ or Q&A content"], gapToExploit: "Their review count is low and response rate is 0%. A review campaign would push you past them within 60 days.", websiteScore: 52 },
          { name: names[2], label: names[2], threat: "low", overallScore: 38, gbpScore: 41, reviewCount: 12, averageRating: 3.9, estimatedMonthlyTraffic: 290, topKeywords: [`security cameras`, `alarm systems`], strengths: ["Established local name"], weaknesses: ["3.9 star rating actively hurting them", "Website has major technical issues"], gapToExploit: "Their poor reviews are driving customers away — target 'alarm company reviews' keywords to capture those searches.", websiteScore: 34 }
        ],
        ...
        biggestOpportunity: "Surrounding city pages — none of the three competitors have content targeting O'Fallon, St. Peters, or Wentzville. Creating city pages for these areas could generate 400+ new monthly searches within 90 days.",
        followUpEmail: {
          subject: `What we found about your competitors in ${city || "your market"}`,
          body: `Hi [Name],\n\nI wanted to share something we found when analyzing your local market.\n\nYour top competitor has 94 Google reviews and dominates the local 3-Pack — but they have zero content targeting surrounding cities. That's a wide open door.\n\nMore importantly, we found 3 specific gaps that could move you past them within 90 days without competing head-to-head on their strongest terms.\n\nWould you have 20 minutes this week to walk through what we found?\n\n[Your Name]`
        }
      });
    }
    setRunning(false);
  ...
  return (
    <div style={{ maxWidth: 620, fontFamily: "var(--font-sans)" }}>
      <div style={{ background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, padding: "14px 16px", marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 11, color: "var(--color-text-secondary)", margin: 0, lineHeight: 1.5 }}>
            {mode === "named" ? "Full competitor profiles with real business names, scores, gaps, and exploitation strategy." : "Competitor profiles labeled A/B/C — creates urgency in prospect presentations without giving away a DIY roadmap."}
          </p>
        </div>
        <button onClick={run} disabled={running} style={{ padding: "8px 14px", background: running ? "transparent" : MODULE_COLOR, border: `0.5px solid ${MODULE_COLOR}`, borderRadius: 6, color: running ? MODULE_COLOR : "#fff", fontSize: 12, fontWeight: 500, cursor: running ? "not-allowed" : "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
          {running ? "Analyzing..." : result ? "Re-run →" : "Analyze Competitors →"}
        </button>
      </div>
                ...
                <div onClick={() => setExpanded(expanded === i ? null : i)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", cursor: "pointer", background: `${tc[c.threat]}06` }}>
                  ...
                  <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 500, color: "var(--color-text-primary)" }}>{c.reviewCount} ★{c.averageRating}</div>
                    </div>
                  ...
                  <div style={{ padding: "10px 12px", borderTop: "0.5px solid var(--color-border-tertiary)" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                      <div>
                        ...
                        {(c.strengths || []).map((s, j) => <div key={j} style={{ fontSize: 10, color: "var(--color-text-secondary)", marginBottom: 2 }}>· {s}</div>)}
                      </div>
                      <div>
                        <div style={{ fontSize: 9, color: "#F87171", fontWeight: 500, marginBottom: 4 }}>THEIR WEAKNESSES</div>
                        {(c.weaknesses || []).map((w, j) => <div key={j} style={{ fontSize: 10, color: "var(--color-text-secondary)", marginBottom: 2 }}>· {w}</div>)}
                      </div>
                    </div>
      ...
      {!result && !running && <div style={{ textAlign: "center", padding: "36px 20px", background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, color: "var(--color-text-secondary)", fontSize: 12 }}>Full competitor profiles — strengths, weaknesses, and exactly how to take their traffic</div>}
    </div>
  );
}
